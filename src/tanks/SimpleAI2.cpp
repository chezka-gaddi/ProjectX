 ...
 */
direction SimpleAI2::move(const MapData &map, PositionData status)
{
 ...
 */
direction SimpleAI2::attack(const MapData &map, PositionData status)
{
    direction ret = STAY;
 ...
 */
attributes SimpleAI2::setAttribute(int pointsAvailable, attributes baseStats)
{
 ...
 */
int SimpleAI2::calcDist(int x1, int y1, int x2, int y2)
{
    return (std::abs(x1 - x2) + std::abs(y1 - y2));
}


int SimpleAI2::spendAP(const MapData &map, PositionData status)
{
    spend = false;
