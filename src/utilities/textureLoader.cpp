...
const std::vector<std::string> bImages = {"images/bush/bush.png"};
const std::vector<std::string> rImages = {"images/rock/rock.png"};
const std::vector<std::string> sImages = {"images/seffect/bTrail.png","images/seffect/bTrail2.png","images/seffect/bTrail3.png","images/seffect/bTrail4.png","images/seffect/campfire.png","images/seffect/explosiona.png","images/seffect/explosionsb.png","images/seffect/explosionsc.png","images/seffect/fire.png","images/seffect/fire2.png","images/seffect/fire3.png","images/seffect/marsh.png","images/seffect/smoke.png","images/seffect/smoke2.png","images/seffect/smoke3.png","images/seffect/smoke4.png"};
const std::vector<std::string> tImages = {"images/tree/tree.png","images/tree/treeS.png","images/tree/trees.png"};
const std::vector<std::string> gImages = {"images/UI/background.png","images/UI/green.png","images/UI/heart.png","images/UI/textbox.png","images/UI/bomb.png","images/UI/GAME.png","images/UI/hbar.png","images/misc/ammo.png","images/misc/hedgehog.png"}; 
                   ...
                   std::vector <std::string> bushes, std::vector <std::string> waters, bool quiet)
{
    ...
    if(!quiet)
      printf("   Loading System UI Textures...\n");
    if (!loadImageGroup(gImages, gameTex, 0, gImages.size(), true))
      return false;
    //Load in the SFX texture information\n
    if(!quiet)
        printf("   Loading System SFX Textures...\n");
    if (!loadImageGroup(sImages, sfxTex, 0, sImages.size(), true))
        return false;
    //Load in the tank texture information
    if(!quiet)
        printf("   Loading Tank Textures...\n");
    if (!loadImageGroup(images, tankTex, 0, images.size(), false))
        return false;
    // Load in the tree texture information
    if(!quiet)
        printf("   Loading User Tree Textures...\n");
    if (!loadImageGroup(trees, treeTex, 0, trees.size(), quiet))
        return false;
    if(!quiet)
        printf("   Loading System Tree Textures...\n");
    if (!loadImageGroup(tImages, treeTex, trees.size(), trees.size() + tImages.size(), true))
        return false;
    // Load in the rock textures information
    if(!quiet)
        printf("   Loading User Rock Textures...\n");
    if (!loadImageGroup(rocks, rockTex, 0, rocks.size(), quiet))
        return false;
    if(!quiet)
        printf("   Loading System Rock Textures...\n");
    if (!loadImageGroup(rImages, rockTex, rocks.size(), rocks.size()+rImages.size(), true))
        return false;
    // Load in the bush texture information
    if(!quiet)
        printf("   Loading User Bush Textures...\n");
    if (!loadImageGroup(bushes, bushTex, 0, bushes.size(), quiet))
        return false;
    if(!quiet)
        printf("   Loading System Bush Textures...\n");
    if (!loadImageGroup(bImages, bushTex, bushes.size(), bushes.size()+bImages.size(), false))
        return false;
...
bool loadImageGroup(std::vector <std::string> source, GLuint * imgArray, int start, int stop, bool quiet){
    if (!quiet)
        printf("      "); //add buffering for loading screen
    ...
    if (!quiet) //add spacing for end of group
        printf("\n");
    return true;
}