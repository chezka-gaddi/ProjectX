...
TEST_CASE("SimpleAI does not attack when no other actors are present")
{
    ...
    map.tileMap[1][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 1, 1, 1, 1)); //place tank on the map
...
TEST_CASE("SimpleAI attacks the nearest actor")
{
    ...
    map.tileMap[1][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 1, 1, 1, 1)); //place tank on the map
    map.tileMap[2][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 2, 1, 2, 1)); //place tank on the map
...
TEST_CASE("SimpleAI moves toward the nearest enemy tank")
{
    ...
    PositionData pos(1, 1, 1, 1, 2); //ID of 1, health of 1, position 0,0
    map.tileMap[1][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 1, 1, 1, 1)); //place tank on the map
    map.tileMap[3][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 2, 1, 3, 1)); //place tank on the map
...
TEST_CASE("SimpleAI stays in place if only projectiles are present")
{
    ...
    PositionData pos(1, 1, 1, 1, 2); //ID of 1, health of 1, position 1,1
    map.tileMap[1][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 1, 1, 1, 1)); //place tank on the map
    map.tileMap[3][1].projectile = std::shared_ptr<Tile>(new Tile("Projectile", -2, 1, 1, 1)); //place tank on the map
...
TEST_CASE("SimpleAI does not crash into other actors")
{
    ...
    PositionData pos(1, 1, 1, 1, 2); //ID of 1, health of 1, position 1,1
    map.tileMap[1][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 1, 1, 1, 1)); //place tank on the map
    map.tileMap[2][1].actor = std::shared_ptr<Tile>(new Tile("Tank", -2, 2, 1, 1)); //place tank on the map
...
TEST_CASE("SimpleAI avoids obstacles below")
{
    ...
    map.tileMap[2][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 1, 2, 2, 1)); //place tank on the map
    map.tileMap[2][3].actor = std::shared_ptr<Tile>(new Tile("Tank", 2, 2, 3, 1)); //place tank on the map
...
TEST_CASE("SimpleAI avoids obstacles to the left")
{
    ...
    map.tileMap[2][3].actor = std::shared_ptr<Tile>(new Tile("Tank", 1, 3, 2, 1)); //place tank on the map
    map.tileMap[2][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 2, 1, 2, 1)); //place tank on the map
    map.tileMap[2][2].type = "Rock";
...
TEST_CASE("SimpleAI avoids obstacles above")
{
    ...
    map.tileMap[3][2].actor = std::shared_ptr<Tile>(new Tile("Tank", 1, 2, 3, 1)); //place tank on the map
    map.tileMap[1][2].actor = std::shared_ptr<Tile>(new Tile("Tank", 2, 2, 1, 1)); //place tank on the map
    map.tileMap[2][2].type = "Rock";
...
TEST_CASE("SimpleAI avoids obstacles to the right")
{
    ...
    map.tileMap[2][1].actor = std::shared_ptr<Tile>(new Tile("Tank", 1, 2, 1, 1)); //place tank on the map
    map.tileMap[2][3].actor = std::shared_ptr<Tile>(new Tile("Tank", 2, 2, 3, 1)); //place tank on the map
    map.tileMap[2][2].type = "Rock";
    REQUIRE((ai.move(map, pos) == UP || ai.move(map,pos) == DOWN));
}
